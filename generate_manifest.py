...
def scan_all_manifests(talon_root: str) -> tuple:
    ...
    Returns:
        - entity_to_package: dict mapping entity names to package info
        - manifest_count: number of manifests found
        - package_deps_map: dict mapping package names to their dependencies
        - package_pip_deps_map: dict mapping package names to their pipDependencies
    ...
    for root, dirs, files in os.walk(talon_root):
        ...
        if 'manifest.json' in files:
            ...
            try:
                ...
                if deps:
                    # Filter out any existing required_by fields from stored deps
                    clean_deps = {}
                    for dep_name, dep_info in deps.items():
                        if isinstance(dep_info, dict):
                            clean = {k: v for k, v in dep_info.items() if k != 'required_by'}
                            clean_deps[dep_name] = clean
                        else:
                            clean_deps[dep_name] = {'min_version': dep_info}
                    package_deps_map[package_name] = clean_deps

                # Index pip dependencies for transitive resolution
                pip_deps = manifest.get('pipDependencies', {})
                if pip_deps:
                    package_pip_deps_map[package_name] = pip_deps
            except Exception as e:
...
def resolve_transitive_dependencies(
    direct_deps: dict,
    package_deps_map: dict,
    current_package: str = None
) -> tuple[dict, list[str]]:
    """
    Walk the dependency tree to find all transitive dependencies.
    Merges them into direct_deps with a 'required_by' field for transitive ones.
    Bumps min_version to the highest required across all paths.

    Returns:
        - all_deps: merged dict of direct + transitive dependencies
        - warnings: list of warning messages (e.g., version bumps)
    """
    warnings = []
    all_deps = {}

    # Copy direct deps (without required_by)
    for pkg_name, pkg_info in direct_deps.items():
        all_deps[pkg_name] = {k: v for k, v in pkg_info.items() if k != 'required_by'}
    ...
    # Queue: (package_name, required_by_chain)
    queue = list(direct_deps.keys())
    ...
    while queue:
        ...
        for sub_dep_name, sub_dep_info in sub_deps.items():
            ...
            sub_version = sub_dep_info.get('min_version', '0.0.0')

            if sub_dep_name in all_deps:
                # Already in our deps (direct or previously found transitive)
                existing = all_deps[sub_dep_name]
                existing_version = existing.get('min_version', '0.0.0')

                # Bump version if the transitive path requires higher
                if compare_versions(sub_version, existing_version) > 0:
                    if 'required_by' not in existing:
                        # This is a direct dep being bumped by a transitive requirement
                        warnings.append(
                            f"bumping {sub_dep_name} min_version from "
                            f"{existing_version} to {sub_version} "
                            f"(required by {dep_name})"
                        )
                    all_deps[sub_dep_name]['min_version'] = sub_version

                # Add to required_by if this is a transitive dep
            ...
            if sub_dep_name not in visited:
                visited.add(sub_dep_name)
                queue.append(sub_dep_name)

    return dict(sorted(all_deps.items())), warnings


def resolve_transitive_pip_dependencies(
    direct_pip_deps: dict,
    direct_git_deps: dict,
    package_deps_map: dict,
    package_pip_deps_map: dict,
    current_package: str = None
) -> dict:
    """
    Walk the git dependency tree to collect pip dependencies from each package.
    Direct pip deps (declared by this package) have no required_by.
    Transitive pip deps (from dependencies) get a required_by array.

    Returns:
        - all_pip_deps: merged dict of direct + transitive pip dependencies
    """
    all_pip_deps = {}

    # Copy direct pip deps (without required_by)
    for pip_name, pip_info in direct_pip_deps.items():
        all_pip_deps[pip_name] = {k: v for k, v in pip_info.items() if k != 'required_by'}

    # BFS through git dependency tree collecting pip deps
    visited = set()
    if current_package:
        visited.add(current_package)

    queue = list(direct_git_deps.keys())
    visited.update(queue)

    while queue:
        dep_name = queue.pop(0)

        # Collect pip deps from this git dependency
        dep_pip_deps = package_pip_deps_map.get(dep_name, {})
        for pip_name, pip_info in dep_pip_deps.items():
            if pip_name in all_pip_deps:
                existing = all_pip_deps[pip_name]
                # Add to required_by if this is a transitive pip dep
                if 'required_by' in existing and dep_name not in existing['required_by']:
                    existing['required_by'].append(dep_name)
            else:
                # New transitive pip dependency
                all_pip_deps[pip_name] = {
                    'version': pip_info.get('version', '*'),
                    'required_by': [dep_name],
                }

        # Continue walking git dep tree
        sub_deps = package_deps_map.get(dep_name, {})
        for sub_dep_name in sub_deps:
            if sub_dep_name not in visited:
                visited.add(sub_dep_name)
                queue.append(sub_dep_name)

    return dict(sorted(all_pip_deps.items()))
...
def create_or_update_manifest(skip_version_errors: bool = False, dry_run: bool = False, verbose: bool = False, output_manifest_path: str = None) -> None:
    ...
    package_deps_map = None
    package_pip_deps_map = None
    manifest_count = 0
    total_warnings = 0
    total_errors = 0
        ...
        if os.path.isdir(full_package_dir):
            ...
            if has_dependencies:
                ...
                if entity_to_package is None:
                    ...
                    entity_to_package, manifest_count, package_deps_map, package_pip_deps_map = scan_all_manifests(talon_user_dir)
                    if verbose:
                ...
                if package_deps_map:
                    package_dependencies, transitive_warnings = resolve_transitive_dependencies(
                        package_dependencies, package_deps_map, current_pkg_name
                    )
                    for warning in transitive_warnings:
                        from diff_utils import status_warning
                        print(status_warning(f"WARNING: {warning}"))
                        total_warnings += 1
                ...
                for pkg_name in existing_deps:
                    if pkg_name in package_dependencies:
                        # Preserve required_by before applying user overrides
                        saved_required_by = package_dependencies[pkg_name].get('required_by')
                        if isinstance(existing_deps[pkg_name], str):
                            ...
                            # Filter out required_by from existing manifest (auto-generated)
                            user_fields = {k: v for k, v in existing_deps[pkg_name].items() if k != 'required_by'}
                            package_dependencies[pkg_name].update(user_fields)
                        ...
                        # Restore required_by (auto-generated, not user-editable)
                        if saved_required_by is not None:
                            package_dependencies[pkg_name]['required_by'] = saved_required_by

                # Enforce highest min_version across transitive requirements
                # A dep's sub-deps may require a higher version of a shared dependency
                if package_deps_map:
                    for dep_name, dep_info in list(package_dependencies.items()):
                        sub_deps = package_deps_map.get(dep_name, {})
                        for sub_dep_name, sub_dep_info in sub_deps.items():
                            if sub_dep_name in package_dependencies:
                                required_version = sub_dep_info.get('min_version', '0.0.0')
                                current_version = package_dependencies[sub_dep_name].get('min_version', '0.0.0')
                                if compare_versions(required_version, current_version) > 0:
                                    from diff_utils import status_warning
                                    print(status_warning(
                                        f"WARNING: bumping {sub_dep_name} min_version from "
                                        f"{current_version} to {required_version} "
                                        f"(required by {dep_name})"
                                    ))
                                    package_dependencies[sub_dep_name]['min_version'] = required_version

            # Resolve pip dependencies (direct from manifest + transitive from git deps)
            existing_pip_deps = existing_manifest_data.get("pipDependencies", {})
            direct_pip_deps = {k: {kk: vv for kk, vv in v.items() if kk != 'required_by'} for k, v in existing_pip_deps.items()}
            pip_dependencies = resolve_transitive_pip_dependencies(
                direct_pip_deps, package_dependencies, package_deps_map or {}, package_pip_deps_map or {}, current_pkg_name
            )
